...
exports.handler = async (event) => {
  ...
  try {
    ...
    const pitchers = [...byPitcher.values()].map(p => {
      ...
      return {
        playerId: p.playerId, playerName: p.playerName, normalizedName: String(p.playerName || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9]/g, ""), pitches: p.pitches, pa: p.pa, bbe: p.bbe,
