...
exports.handler = async (event) => {
  ...
  try {
    ...
    const params = new URLSearchParams({
      type: "details", player_type: "pitcher", game_date_gt: startDate, game_date_lt: endDate,
      hfGT: "R|", min_pitches: "0", min_results: "0", group_by: "name-date"
    });
    const upstream = await fetch(`${SAVANT_CSV}?${params.toString()}`, {
      headers: { Accept: "text/csv,*/*", "User-Agent": "Mozilla/5.0 (compatible; RealBodmonHR/5.4)" }
    });
    ...
    if (rows.length < 2) return { statusCode: 200, headers: { "Content-Type": "application/json", "Cache-Control": "no-store" }, body: JSON.stringify({ startDate, endDate, days, pitchers: [] }) };

    const header = rows[0].map(h => h.trim());
    const idx = Object.fromEntries(header.map((h, i) => [h, i]));
    for (const col of ["game_date", "pitcher", "events", "plate_x", "plate_z", "launch_speed", "launch_speed_angle", "bb_type"]) {
    ...
    for (let r = 1; r < rows.length; r++) {
      const row = rows[r];
      const gameDate = String(row[idx.game_date] || "").trim();
      ...
      if (!p) {
        p = { playerId, pitches: 0, zone: 0, heart: 0, meatball: 0, pa: 0, homeRuns: 0, bbe: 0, barrels: 0, hardHit: 0, hardAir: 0 };
    ...
    const pitchers = [...byPitcher.values()].map(p => {
      ...
      return {
        playerId: p.playerId, pitches: p.pitches, pa: p.pa, bbe: p.bbe,
    ...
    return { statusCode: 200, headers: { "Content-Type": "application/json", "Cache-Control": "no-store, max-age=0, s-maxage=0" }, body: JSON.stringify({ startDate, endDate, days, pitchers }) };
  } catch (error) {
