...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Baseball Savant uses exclusive gt/lt date bounds. Expand the upstream
    // request by one day on each side, then enforce the intended inclusive
    // window locally below.
    const queryStartDate = addDays(startDate, -1);
    const queryEndDate = addDays(endDate, 1);
    // Request pitch-level detail rows. Do not group by player name.
    const season = String(Number(date.slice(0, 4)));
    const params = new URLSearchParams({
      all: "true",
      type: "details",
      player_type: "pitcher",
      game_date_gt: queryStartDate,
      game_date_lt: queryEndDate,
      hfSea: `${season}|`,
      hfGT: "R|PO|S|",
      min_pitches: "0",
      min_results: "0",
    ...
    // Data for past days (everything except "today", which this window
    // already excludes) does not change intraday, so short-lived CDN
    // caching is safe and cuts repeated full-league Savant pulls when the
    // page is loaded more than once for the same slate date within a
    // few minutes (e.g. re-running after toggling lineup status).
    const CACHE_HEADERS = { "Content-Type": "application/json", "Cache-Control": "public, max-age=300, s-maxage=900, stale-while-revalidate=300" };
    if (rows.length < 2) return { statusCode: 200, headers: CACHE_HEADERS, body: JSON.stringify({ startDate, endDate, days, pitchers: [], debug: { source: SAVANT_CSV, query: params.toString(), contentType: upstream.headers.get("content-type"), bytes: rawText.length, parsedRows: Math.max(0, rows.length - 1), pitcherCount: 0, rawPreview: rawText.slice(0, 500) } }) };
    ...
    const seenBattedBalls = new Set();
    const seenGamesByPitcher = new Map(); // playerId -> Set of game_pk, to count starts in-window
    for (let r = 1; r < rows.length; r++) {
      ...
      const gamePk = idx.game_pk !== undefined ? String(row[idx.game_pk] || "").trim() : "";
      if (gamePk) {
        if (!seenGamesByPitcher.has(playerId)) seenGamesByPitcher.set(playerId, new Set());
        seenGamesByPitcher.get(playerId).add(gamePk);
      }
      const atBatNumber = idx.at_bat_number !== undefined ? String(row[idx.at_bat_number] || "").trim() : "";
      const pitchNumber = idx.pitch_number !== undefined ? String(row[idx.pitch_number] || "").trim() : "";
      const paKey = gamePk && atBatNumber ? `${gamePk}:${atBatNumber}` : `${playerId}:${gameDate}:${r}:pa`;
      const pitchKey = gamePk && atBatNumber && pitchNumber ? `${gamePk}:${atBatNumber}:${pitchNumber}` : `${playerId}:${gameDate}:${r}:pitch`;
      if (seenPitches.has(pitchKey)) continue;
      ...
      if (eventName && !seenPlateAppearances.has(paKey)) {
        seenPlateAppearances.add(paKey);
        p.pa++;
        if (eventName === "home_run") p.homeRuns++;
      }
      const ev = num(row[idx.launch_speed]);
      if (ev === null || seenBattedBalls.has(pitchKey)) continue;
      seenBattedBalls.add(pitchKey);
      p.bbe++;
    ...
    const pitchers = [...byPitcher.values()].map(p => {
      ...
      const starts = seenGamesByPitcher.get(p.playerId)?.size ?? null;
      ...
      // Weighting was revised after finding a pitcher (Bryce Elder, Aug 2026) who
      // allowed a home run in 4 straight starts (7 HR / ~90 PA, a clearly
      // homer-prone stretch) but still scored a near-neutral Attack Score,
      // because process/location metrics (zone/heart/meatball/barrel/hard-hit)
      // happened to look ordinary even during that stretch. Actual home runs
      // allowed is the most direct signal of what this score is trying to
      // predict, so it now carries double its old weight (15% -> 30%),
      // taken proportionally from the location-based metrics.
      const raw = zoneScore * .05 + heartScore * .15 + meatballScore * .15 + barrelScore * .15 + hardHitScore * .10 + hardAirScore * .10 + hrScore * .30;
      ...
      return {
        ...
        homeRuns: p.homeRuns, hrPer100PA: round(hrPer100PA), starts,
        attackScore: round(clamp(attackScore), 0)
      };
    });
    ...
    const debug = {
      ...
      sampleDates,
      uniquePitches: seenPitches.size,
      uniquePlateAppearances: seenPlateAppearances.size,
      uniqueBattedBalls: seenBattedBalls.size,
    ...
    return { statusCode: 200, headers: CACHE_HEADERS, body: JSON.stringify({ startDate, endDate, days, pitchers, debug }) };
  } catch (error) {
