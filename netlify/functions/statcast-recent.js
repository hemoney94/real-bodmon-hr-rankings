...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Request pitch-level detail rows. Do not group by player name; grouping can
    // cause repeated or aggregated rows that inflate batted-ball totals.
    ...
    if (rows.length < 2) {
      return {
        ...
        body: JSON.stringify({ startDate, endDate, days, players: [], debug: { source: SAVANT_CSV, query: params.toString(), contentType: upstream.headers.get("content-type"), bytes: rawText.length, parsedRows: Math.max(0, rows.length - 1), playerCount: 0, rawPreview: rawText.slice(0, 500) } })
      };
    }
